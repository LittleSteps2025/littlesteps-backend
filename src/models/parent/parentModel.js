...
export const findById = async (id) => {
  const query =
    "SELECT id, email, name, verified, created_at, updated_at FROM parent WHERE id = $1";
...
export const getAllParents = async () => {
  const query = `
    SELECT u.* , COUNT(c.child_id) AS child_count
    FROM "user" u 
...
export const getVerifiedParentByEmail = async (email) => {
  try {
    ...
    if (
      ...
      verificationResult.rows[0].verified === false
    ) {
