...
// Create medical record (public - no auth)
export const createMedicalRecord = async (req, res) => {
  try {
    ...
    const existingRecord = await getRecordByChildAndDate(Number(child_id), record_date);
    if (existingRecord) {
      return res.status(409).json({
        success: false,
        message: 'Conflict - Record already exists',
        errors: { record_date: ['A medical record for this date already exists'] }
      });
    }

    const created = await insertMedicalRecord({ 
...
export const updateMedicalRecord = async (req, res) => {
  try {
    ...
    const existing = await getRecordByChildAndDate(Number(child_id), record_date);
    ...
    if (!existing) {
      return res.status(404).json({ 
        ...
        errors: { record: ['No medical record found for the provided child ID and date'] }
      });
    }
